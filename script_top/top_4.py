...
def generate_top_mostExpensive_csv(listaGeneros, output_path, number_choice):
    spark = SparkSession.builder.appName("TopmostExpensive").getOrCreate()

    file_path = "Google-Playstore.csv"
    df = spark.read.option("header", "true").csv(file_path)

    filtered_df = df.filter(col("Category").isin(listaGeneros) & (col("Free") == "False"))
    sorted_df = filtered_df.orderBy(col("Price").cast("float").desc())

    top_df = sorted_df.limit(number_choice)
    result_df = top_df.select("App Name", "Price", "Category")
...
output_path = "output_top/Top_mostExpensive.csv"
generate_top_mostExpensive_csv(listaGeneros, output_path, number_choice)
